...
@section('description')
Edit a News Category for Your Website
@endsection

@section('content')
<div class="mt-3 card">
    <div class="card-body">
        <form method="POST" action="{{ route('newsCategories.update', ['newsCategory' => $category->slug]) }}">
            @method('PUT')
            @csrf
            <div class="mb-3">
...
<script>
    ...
    function checkSlugUniqueness(slug) {
        fetch('/dashboard/newsCategories/checkSlug?slug=' + slug)
